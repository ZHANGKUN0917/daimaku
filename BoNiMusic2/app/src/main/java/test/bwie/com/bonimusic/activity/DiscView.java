...
public class DiscView extends RelativeLayout {
    ...
    private List<SmBean> mMusicDatas = new ArrayList<>();
    ...
    private Drawable getDiscDrawable(String musicPicRes) {
        ...
        Bitmap bitmapMusicPic = getMusicPicBitmap(musicPicSize,R.mipmap.ic_launcher);
    ...
    private Bitmap getMusicPicBitmap(int musicPicSize, int musicPicRes) {
        ...
        return Bitmap.createScaledBitmap(BitmapFactory.decodeResource(getResources(),
                R.mipmap.ic_launcher, options), musicPicSize, musicPicSize, true);
    }

    public void setMusicDataList(List<SmBean> musicDataList) {
        ...
        int i = 0;
        for (SmBean musicData : mMusicDatas) {
            ...
            disc.setImageDrawable(getDiscDrawable(musicData.getAlbum()));
        ...
        SmBean smBean = mMusicDatas.get(0);
        if (mIPlayInfo != null) {
            mIPlayInfo.onMusicInfoChanged(smBean.getName(), smBean.getAuthor());
            mIPlayInfo.onMusicPicChanged(smBean.getAlbum());
        }
    }
    ...
    /*播放唱盘动画*/
    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public void playDiscAnimator(int index) {
    ...
    /*暂停唱盘动画*/
    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public void pauseDiscAnimatior(int index) {
        ObjectAnimator objectAnimator = mDiscAnimators.get(index);
    ...
    public void notifyMusicInfoChanged(int position) {
        if (mIPlayInfo != null) {
            SmBean bean = mMusicDatas.get(position);
            mIPlayInfo.onMusicInfoChanged(bean.getName(),bean.getAuthor());
        }
    }
    ...
    public void notifyMusicPicChanged(int position) {
        if (mIPlayInfo != null) {
            SmBean bean  = mMusicDatas.get(position);
            mIPlayInfo.onMusicPicChanged(bean.getAlbum());
        }
    }
